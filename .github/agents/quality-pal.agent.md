...
**Using knowledgebase-wizard for best-practice lookups:**
When a review uncovers a question about library usage, framework best practices, or dependency security, delegate to `@knowledgebase-wizard` with the `[AGENT-CALL]` prefix for compact structured results:
```bash
copilot --agent knowledgebase-wizard -p "[AGENT-CALL] <your query>"
```
